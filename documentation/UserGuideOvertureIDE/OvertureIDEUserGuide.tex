...
\noindent These can then be added to the Java build path, as shown in Figure~\ref{fig:gui:VDMJBuildPath}. The alternative is to use the command line \texttt{Overture.x.x.x.Jar} file and add that to the Java build path, since this contains all of the above Jars in a single file.
