...
\begin{document}
\title{Overture VDM-10 Tool Support: User Guide \\{\large Version 2.3.0}}
...
to export the UML model from Modelio to the Eclipse XMI\index{XMI}
format, called UML using the EMF UML3.0.0 format. At the moment,
Modelio\index{Modelio} is the only UML tool supported. In particular,
...
\chapter{Graphics Plotting while Interpreting VDM Model}\label{sec:graphics}

In some occasions it can be convenient to be able to monitor how numeric variables inside a VDM++ model evolve over time. A plugin extension to Overture enable such a graphics plotting while a VDM model is being interpreted. However, it is only possible to expose instance variables that contains numeric values and that have been declared static and public so they can be exposed externally in this fashion. In order to be able to make use of this feature it is necessary to instal the ``Overture Graphics Plugin'' through Overture's``Install New Software \ldots '' feature. The repository URL is:\\

\noindent \url{http://overture.au.dk/overture-graphics-plugin/master/repository/}\\

When that has been imported it is possible to start the graphics plugin as a special debug configuration as shown in Figure~\ref{fig:graphicsdebug}. 
...
This will then automatically add a \texttt{overture.graphics.properties} file and a file called \texttt{overture.graphics.jar} in the \texttt{lib} directory and these files are essentially internally making use of the remote control feature described in Section~\ref{sec:Remote} below. This will then start up a new window as the one shown in Figure~\ref{fig:graphicselectclass} where the user first needs to select the root class to use for the debugging and afterwards the main operation to be invoked. Note that both of these cannot have any parameters.
...
\end{figure}
 ...
 Afterwards one can define different plots as shown in Figure~\ref{graphicsAddPlot} where one is able to select numeric instance variables that have been declared both static and public. This new application started up is in fact a separate application making using the Electron framework. From the file menu it is also possible to save such plot configurations such that one can load them in a later session instead of having to define them again. Numerous plots can be defined if desired but typically it is advantageous to display multiple graphs on the same plot. 
...
When one is finished with configuring it is possible to see the actual plot by clicking on the small plus in front of the selected type of plot (in this case a 2D plot) and then double-click on the plot one would like to see. Then one can start the entire simulation by pressing the Start bottom. This will invoke the VDM interpreter with the selected class and operation and as time is advancing the plot is automatically updated for all the selected instance variables. Such a plot looks like what is shown in Figure~\ref{fig:graphicsplot}.

\begin{figure}[htbp]
\begin{center}
\includegraphics[width=\textwidth]{screenDumps/graphicsplot}
\caption{Starting up the debugger using the Graphics plugin\label{fig:graphicsplot}}
\end{center}
\end{figure}
