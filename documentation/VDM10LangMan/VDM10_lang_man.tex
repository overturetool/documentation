...
\item[Semantics:] Operations in VDM are per default synchronous but if
  ...
  declared asynchronous. If an operation is declared to be ``\keyw{pure}"
  it means that it is executed atomically if it is called from a functional context (from normal functions, invariants, pre or post-conditions). Such operations cannot call ordinary operations nor update the instance variables, i.e.\ have any side effects, and thus they can also not be used in synchronisation constraints.
