...
Whenever an equality relation is defined the proof obligation is that it be an equivalence relation; that is, it is reflexive, symmetric and transitive.
...
\begin{tabular}{l l }
{\tt x <= y} & {\tt <=> x < y or x = y} \\
{\tt x > y} & {\tt <=> not (x = y or x < y)} \\
{\tt x >= y} & {\tt <=>  x = y or not x < y}\\
\end{tabular}

The proof obligation for an order relation is that it be a strict partial order; that is, it is irreflexive and transitive.
\begin{description}
\item Irreflexive:
\begin{lstlisting}
forall x:T & not x < x 
