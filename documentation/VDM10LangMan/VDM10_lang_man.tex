...
\item[Semantics:] Operations in VDM are per default synchronous but if
  ...
  operation, except that a pure operation has various constraints: A
  \keyw{pure} operation cannot call ordinary operations nor update
  instance variables, i.e.\ have any side effects. As a consequence a
  pure operation cannot be used in synchronisation
  constraints. Functions, on the other hand, are always atomic - which
  is consistent with them being able to read instance variables of
  objects passed, and there being no ``\keyw{sync}'' clauses for
  functions.
