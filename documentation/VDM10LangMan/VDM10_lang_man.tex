...
May & 2020 & 12 & 2.7.4 & Includes RM \#44,\#47,\#48 \\ \hline
July & 2021 & 13 & 3.0.2 & Includes RC \#52 \\ \hline
December & 2022 & 14 & VSCode plugin 1.3.7 & Includes RC \#49 \\ \hline
November & 2023 & 15 & VSCode plugin 1.3.7 & Includes RC \#50 \\ \hline
\end{tabular}
...
\item[Equation:] {\tt F = A +> B} or {\tt F = A -> B}%
...
Defining a measure for a function {\tt f} with signature {\tt f : T1 * } \ldots {\tt * Tn -> T} causes creation of a function {\tt measure\_f} with signature
{\tt measure\_f : T1 * }\ldots{\tt * Tn +> nat} or
{\tt measure\_f : T1 * }\ldots{\tt * Tn +> nat * }\ldots{\tt * nat}.
...
are the same as the recursive function's parameters. For
...
\item[Semantics:] A simple {\it let expression} has the form:
  ...
  recursive.  However, this is not supported by the VDM interpreters.  
  Furthermore, the definitions must be ordered such that all constructs are defined before they are used.
...
\item[Semantics:] A {\it define expression} has the form:
  ...
  that binding the local identifiers and the evaluations of the right hand side
  expressions are processed in the order in which they are presented. 
  Therefore,  right hand side expressions may not be mutually recursive.
  A local identifier may be referenced in its following right hand side expressions
  and also may be overriden by its following bindings.

  A define expression denotes the value of the expression {\tt e} in the
...
\item[Examples:] The {\it define expression} is
  ...
  the local bindings are defined and/or overriden in order.  
  ...
  \begin{lstlisting}
    ...
    in
      x
  \end{lstlisting}
  where {\tt op} is an operation that may modify state variables.
  A {\tt define expression} may appear in the body of a non-pure operation, and in such cases its right hand side expressions may contain operation calls and read access to state variables.
  If in the body of a pure operation, the right hand side expressions may contain pure application calls and read access to state variables.

  In the above example,  {\tt op(1)} is first evaluated and {\tt x} gets its return value, and then {\tt op(x)} is evaluated and {\tt x} now gets its return value.
  The entire expression evaluates the resulting value of {\tt x}.
...
\item[Syntax:]
  ...
  \Rule{sequence comprehension}{
    \Lit{[}, \Ruleref{expression}, \Lit{|}, \Ruleref{bind}, 
    \OptPt{\Lit{\char'046}, \Ruleref{expression}}, \Lit{]}
    }
...
\item[Semantics:] A {\it sequence enumeration} has the form:
  ...
  \begin{lstlisting}
    [e | pat in seq S & P]
  \end{lstlisting}
  where the expression \texttt{e} may refer to the identifiers introduced by the
  one pattern \texttt{pat}. \texttt{S} is a
...
Note that a superclass constructor can be called, but to avoid this
being the return value of the block, it is common to put these inside
\keyw{let} statements:
...
Since a constructor is used for initialising a new instance of a class,
it is not permitted to declare a constructor as \keyw{static}.
...
Object construction in VDM is unusual, in that the default initialization
of object fields is \emph{always} performed first, including superclass
parent objects. This can be influenced if a class provides a \emph{default}
constructor (one with no parameters). Once the default construction has
completed, then the body of the constructor given in the \keyw{new}
expression is called \emph{as a normal operation}. That operation can
explicitly call a superclass constructor, but again this is acting as
a normal operation.

With multiple inheritance, default construction occurs in the order of
the \keyw{is subclass of} clause, from left to right; parent classes are
constructed before subclasses. If a parent hierarchy has a common
superclass (diamond inheritance), the common object is only constructed
once.
...
\item[Semantics:] A {\it define statement} has the form:
  ...
  except that the {\it in} part is a statement instead of an expression. 
  Like the {\it define expression}, operations that change the state can 
  also be used in right hand side expressions, and if there are more than 
  one definition they are evaluated in the order in which they are presented.  
  A define statement denotes the evaluation of the statement {\tt s} 
  in the context in which the patterns (or binds) {\tt pb1, \ldots,  pbn} are 
  matched against the values returned by the corresponding expressions 
  or operation calls {\tt e1, \ldots, en}
  \footnote{If binds are used it simply means that the values which can 
  match the pattern are further constrained by the type or set expression 
  as it is explained in section~\ref{patterns}.}.
...
\item[Examples:] In the following example {\tt OpCall} is an operation call
  ...
  ``converted'' to a statement by using the {\it return statement}, i.e.  {\tt return FunCall()}.
  This ``conversion'' may be carried out explicitly by the specifier, or implicitly by the interpreter.
...
class are to be executed mutually exclusive, an operation is to be executed 
mutually exclusive to itself, or that a list of operations is to be executed mutually 
exclusive to each other.
Operations that appear in one mutex predicate are allowed to
...
As shown in the translated per clauses, a mutex on multiple operations is 
translated to a set of permission predicates each of which blocks the execution 
of an operation when any one of the other operations is being executed. 
Please note that a mutex on multiple operations does not prevent multiple executions 
of any one of the operations. A mutex on a single operation, such as {\bf\ttfamily{mutex(opD)}}
in the above example, blocks multiple executions of the operation.
...
Note that multiple line comments will be parsed in a single level without nesting, but tools may provide different strategies of parsing comments, e.g. it is possible for tools to optionally parse nested comments.
...
Note: the '{\tt =}' operator is in the Relations family, which does not have specified grouping.
However, it is possible to construct a meaningful expression with a chain of '{\tt =}' operators comparing Boolean values, such as {\tt true = 3 = 3}. This is parsed as {\tt true = (3 = 3)} -- i.e. the '{\tt =}' operator has right grouping.

The '{\tt **}' operator is always treated as an iterator for the purpose of grouping, and is therefore a member of the Combinators family and has right grouping.
...
\chapter{Maximal Types}\label{sec:maximaltypes}

As described in section \ref{inv}, \ref{eq} and \ref{ord}, type definitions that include \keyw{inv}, \keyw{eq} or \keyw{ord} clauses generate implicit functions, which can be used elsewhere in a specification. For example, the following type definition generates the implicit functions shown:
...
However, the example above causes problems. Note that the equality and ordering clauses try to reflect the implicit rules for the underlying {\tt nat} value (e.g. the \keyw{eq} comparison for two numbers {\tt t1} and {\tt t2} is just {\tt t1 = t2}). When this is turned into the {\tt eq\_T} or {\tt ord\_T} functions, they would naively be defined as follows:
...
But an evaluation of these functions would produce an infinite recursion, because the body of both functions involves an \keyw{eq}/\keyw{ord} comparison of two {\tt T} variables, which invokes the {\tt eq\_T} / {\tt ord\_T} function, and so on...

A similar situation occurs with the {\tt inv\_T} function, which is passed a value of type {\tt T}. But in order to create a {\tt T} value, the {\tt inv\_T} function must have been called (successfully), so calling {\tt inv\_T(123)} would try to call {\tt inv\_T(123)} to convert the {\tt 123} literal to a {\tt T} value, which also produces an infinite recursion.

The solution to this problem is the use of {\it maximal types}. For a type {\tt T}, there is a maximal type {\tt T!} which is the same, but without any invariant, equality or ordering constraints. If you look closely, the signature of {\tt inv\_T} shows that it actually takes a {\tt T!} parameter. This type can accept any natural number, so the recursion is avoided.
...
Notice that the \keyw{let} clauses convert the parameters to maximal types, which are then used to perform the check as the user specified without recursion.

Normally, maximal types are only used internally in this way, and it is illegal to use {\tt T!} in a specification. However, it may be possible to enable the use of {\tt T!} types in specification, depending on tool support.
