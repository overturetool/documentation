...
\small
\begin{lstlisting}
-- A type describing valid character mappings. The map must be
-- injective, and it must map arbitrary characters onto the set
-- {0, ..., N-1}, where N is the base of the algorithm.
...
\texttt{Mapping} as well as a base and a string of characters, returning a single check digit:
...
The \texttt{luhn} function is passed a sequence of numbers and a base, returning
another number of that base:
...
The precondition checks that the values in the input are within the base. There
is also a recursive measure that checks that the string of data being totalled
is always decreasing in length.
