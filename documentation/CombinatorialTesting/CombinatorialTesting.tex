...
\fancyhead[RE]{VDM Combinatorial Testing Guidelines}
...
January   & 2021 &    1.0     & 3.0.2   & First public release\\ \hline
\end{tabular}
...
grammar of traces.
\item Appendix \ref{chap:tools}, \nameref{chap:tools} describes how to use
...
\noindent As well as defining a variable value from a set, variables can be used to
...
\noindent If repetitions are added to a clause within a \texttt{\textbf{let}} body, they bind
...
a \texttt{?} operator. This produces the same set, but includes a ``\texttt{\textbf{skip}}'' step.
...
\section{Overture GUI}

The Overture IDE includes a perspective that is intended for working with
combinatorial tests in a specifiction. The features available are described in
Chaper 10 of the Overture User Guide.

\begin{center}
\includegraphics[width=10cm]{overturect}
\end{center}

\section{Visual Studio Code GUI}

There is a new experimental VDM extension for Visual Studio Code available that
supports combinatorial testing
\footnote{https://github.com/jonaskrask/vdm-vscode}. The interface should be
intuitive and consistent with normal VS Code usage.

\begin{center}
\includegraphics[width=10cm]{vscode}
\end{center}

\section{VDMTools UI}

VDMTools\footnote{https://github.com/vdmtools/vdmtools} includes traces support
through interpreters. The interpreter window that provides a REPL interface
accepts the \texttt{traces} command to run a trace, although the tool does not
have a GUI dedicated to traces.
...
\begin{description}
\item[\texttt{traces <name>}] This command expands the named trace and runs all of the tests.
\end{description}
...
\scriptsize
\lstinputlisting[language=VDM++]{LUHN/LUHN.vdmpp}
\lstinputlisting[language=VDM++]{LUHN/LUHN_STD.vdmpp}
\lstinputlisting[language=VDM++]{LUHN/LUHN_ACME.vdmpp}
\lstinputlisting[language=VDM++]{LUHN/LUHN_Test.vdmpp}
\normalsize

\section{The Basket Service Model}

\scriptsize
\lstinputlisting[language=VDM++]{BasketService/BasketService.vdmpp}
\lstinputlisting[language=VDM++]{BasketService/Tests.vdmpp}
\normalsize
