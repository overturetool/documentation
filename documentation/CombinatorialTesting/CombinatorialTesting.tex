...
the traces in a module are implicitly exported, so the are referred to as
\texttt{<modulename>`<tracename>}. You can omit the module name if it is the
default module.

The equivalent VDM-SL specification that is equivalent to the example used above
is like this:
...
So this model is very similar to the VDM++ example. The Counter module has a
...
because each test effectively re-initializes the specification, and the module
...
Counter object is created for each test, because the ``obj'' instance variable
...
trace. This is because some expressions are evaluated during expansion and some
...
multi-binds, and this is executed during expansion - once for ``x'' and three
times for ``y''. But the addition of \texttt{Z + x + y} in the argument to
\texttt{inc} is called during execution.
