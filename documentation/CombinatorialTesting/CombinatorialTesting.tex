...
\end{verbatim}
\normalsize
  \item A common requirement is to select names items from a set, such that the
  items form every possible permutation of that set. This can be done by using
  the looseness of a set bind, which expands to such a permutation. Note the use
  of the set pattern and the set-of-set \texttt{\{S\}} in the bind values:
\small
\begin{verbatim}
