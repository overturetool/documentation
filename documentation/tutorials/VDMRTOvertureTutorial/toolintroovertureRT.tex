...
In this chapter, an in-car radio navigation system which supports the Traffic Message Channel (TMC) will used as an example. 
...
from \texttt{.zip} file mentioned above can be imported very easily.  Import the project \texttt{CarNaviRadioRT}.
...
the bottom of the view) are evaluated\footnote{If errors appear in
  ...
  will provide more helpful run-time error descriptions to the user.}. Note that
...
and creating a new VDM-RT project and, naming it, for example,
\texttt{CarNaviRadioUML}. If the desired language dialect (say a VDM-RT 
...
exported from Modelio called \texttt{carradnavuml.uml} and open it.
...
\rightarrow $ \emph{Export XMI} (exported in EMF UML3.0.0 format, with extension \texttt{.uml}). The UML/XMI file can subsequently be
...
must be selected in the debug launch configuration, as shown in Figure~\ref{fig:userguide:debugConfiguration}. 
 Once the debugger
...
The coverage information is provided in such a way that expressions not covered 
...
Inside this combinatorial testing view of the Combinatorial Testing perspective, you can
...
\emph{.rt} extension.
%Note that \texttt{debugconfig} here will be the name of the launch configuration that the user has created, so if several launch configurations are used, these will be separated in different directories.
...
\begin{lstlisting}
java -jar Overture-2.2.6.jar -vdmrt CarNaviRadioRT
\end{lstlisting}
...
\begin{lstlisting}
java -jar Overture-2.2.6.jar -vdmrt -w -q -e 
