...
\item[\textbf{Modelio:}] This is a tool that is both available in a commercial version as well as in an open source setting from a French
...
for the exercises in the book, is available and can be imported as ``Overture examples" directly into the Overture tool. An overview of these can be found at \texttt{http://overturetool.org/download/examples/} as well.
...
to UML for Modelio using a file based interface called XMI.
...
site\footnote{\url{http://overturetool.org/publications/books/vdoos/AlarmInitUML.uml}.}.
...
this \texttt{.uml} file and import it using Modelio. Before being able to import an XMI file in Modelio one need to open a new project. When
...
indicates that this association is \kw{private}. The `\texttt{+}' in
front of the r\^{o}le name \texttt{alarms} indicates that it is
\kw{public}. You can change the visibility of the
\texttt{alarns} association to \kw{private} by selecting the
...
\emph{Overture Examples}.  Initially it is
...
Explorer view by double clicking. 
%When the
...
project \texttt{AlarmUML}. Following this it is possible to choose which project should be referenced but you shall just press next here and then it will be possible
...
\rightarrow $ \emph{Convert to UML}. The XMI file can subsequently be
...
on the \texttt{AlarmPP} project and select the \emph{VDM Settings} $
\rightarrow $ \emph{Latex} as shown in Figure~\ref{fig:testcovopt}.
Afterwards one can again right click the project and select the \emph{Latex} $
...
\begin{figure}[tb]
\begin{center}
\resizebox{.4\textwidth}{!}{\includegraphics{figures/testcoverage}}
\end{center}
\caption{The option for selecting test coverage information to be shown\label{fig:testcovopt}}
\end{figure}
...
an expert to/from the schedule for a particular period. Please remember
